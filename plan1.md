...
POST /chatbot/webhook/incoming/ — Receive message → ChatbotEngine → return responses
POST /chatbot/webhook/pause-session/ — Pause chatbot for human agent
GET /chatbot/webhook/chatbot-phones/ — Return active chatbot phone numbers
...
Verification
Install module in Odoo → verify models created, views render, menus appear
Create a chatbot + flow + nodes via Odoo UI
Configure WhatsApp phone mapping
Point WhatsApp service webhook URL to Odoo
Send WhatsApp message → verify response arrives back through the flow